...
export const generateClubTeamsColumns: ({
  ...
  dayNumber: number
  setDayNumberAction: (day: number) => void
  maxDayNumber: number
  redirectToPoolPageAction: (poolId: string) => void
  defaultCategoryFilter?: string[]
}) => ColumnDef<ClubTeamsTable>[] = ({
  dayNumber,
  setDayNumberAction: setDayNumber,
  maxDayNumber,
  redirectToPoolPageAction,
  defaultCategoryFilter = []
}) => [
  {
    accessorKey: 'name',
        ...
        <div className="text-center justify-center flex items-center gap-2">
          ...
          <Tooltip open={isOpen} onOpenChange={setIsOpen}>
            ...
            <TooltipContent className="bg-neutral-800">
              SD = Senior dame <br />
              SH35 = Senior homme +35 ans <br />
              SH = Senior homme <br />
              G = Garçon <br />
              F = Fille <br />
              11/13 = Catégorie 11/13 ans <br />
            </TooltipContent>
          </Tooltip>
        </div>
      )
      ...
      if (row.getValue('rank') === 1) {
        return (
          <div
            ...
            onClick={() => redirectToPoolPageAction(row.original.pool.id)}
          >
            <Medal className="absolute mr-36 h-4 w-4" />
            {row.getValue('name')}
          </div>
        )
      }
    ...
    cell: ({ row }) => (
      <div
        className={cn(
          ...
          row.original.pool.ballColor !== 'yellow' && 'pl-2 gap-1'
        )}
      >
        {row.original.pool.division}
        {row.original.pool.ballColor === 'orange' && (
          <img src="/ball/orange.png" alt="Balle orange" className="h-7 w-8" />
        )}
        {row.original.pool.ballColor === 'green' && (
          <img src="/ball/green.png" alt="Balle verte" className="h-8 w-8" />
        )}
      </div>
    )
  },
      ...
      if (row.getValue('rank') === 1) {
        return (
          <div className="relative flex justify-center items-center">
            {' '}
            <Medal className="absolute inline-block mr-12 h-4 w-4" />
            1er
          </div>
        )
      }
